...
use crate::{models::BootstrapState, state::{AppState, EmbeddingStatus}};

#[tauri::command]
pub fn healthcheck(state: State<'_, AppState>) -> Result<bool, String> {
    let connection = state.db.lock().map_err(|_| "Database state is unavailable".to_string())?;
    connection
...
pub fn get_bootstrap_state(state: State<'_, AppState>) -> Result<BootstrapState, String> {
    ...
    let stage = connection
        .query_row("SELECT current_stage FROM memory_state WHERE id = 1", [], |row| row.get(0))
        .unwrap_or_else(|_| "clippy".to_string());
    drop(connection);

    let embedding_status = match *state.embedding_status.lock().map_err(|_| "Embedder state is unavailable".to_string())? {
