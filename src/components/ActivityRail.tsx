...
function ActivityItem({ clip }: { clip: Clip }) {
  ...
  return <article className="activity-item"><div className="activity-item-meta"><span>{clip.appName || "Unknown source"}</span><time>{timeAgo(clip.copiedAt)}</time></div><Link to={`/clip/${clip.id}`}>{clip.content.slice(0, 105)}{clip.content.length > 105 ? "…" : ""}</Link><div className="activity-item-actions"><span>{clip.sessionId ? "Research session" : "Unsorted"}</span><div><Link to={`/clip/${clip.id}`} aria-label="Open clip"><ExternalLink size={13} /></Link><button disabled={busy} onClick={() => { void copy(); }} aria-label="Copy clip">{copied ? <Check size={13} /> : <Copy size={13} />}</button><button disabled={busy} onClick={() => { void pin(); }} aria-label={clip.isPinned ? "Unpin clip" : "Pin clip"}><Pin size={13} /></button></div></div></article>;
}
