...
export function Memory() {
  ...
  const [onboardingVisible, setOnboardingVisible] = useState(() => window.localStorage.getItem("mnemo-onboarding-complete") !== "true");
  ...
  if (!clips.length && onboardingVisible) return <section className="page onboarding-page"><div className="onboarding-intro"><p className="eyebrow">Welcome to Mnemo</p><h1 className="page-title">Your clipboard, with memory.</h1><p className="page-copy">Mnemo keeps useful copies on this device, then connects them into research you can actually find again.</p></div><div className="onboarding-steps"><article><span><ShieldCheck size={18} /></span><div><b>Private by default</b><p>Capture is visible in the sidebar and can be paused any time. Browser context stays off until you enable it.</p></div><Check size={16} /></article><article><span><Sparkles size={18} /></span><div><b>Preparing local intelligence</b><p>{bootstrap?.embeddingStatus === "ready" ? "Your local model is ready to connect related clips." : "The local model is preparing in the background. Search still works while it loads."}</p></div><Check size={16} /></article><article><span><Keyboard size={18} /></span><div><b>Two shortcuts, one purpose</b><p><kbd>Ctrl/Cmd + Shift + V</kbd> searches your memory. <kbd>Ctrl/Cmd + Shift + M</kbd> pauses or resumes capture.</p></div><Check size={16} /></article></div><button className="primary-button onboarding-start" onClick={() => { window.localStorage.setItem("mnemo-onboarding-complete", "true"); setOnboardingVisible(false); }}>Start remembering <ArrowRight size={15} /></button></section>;
