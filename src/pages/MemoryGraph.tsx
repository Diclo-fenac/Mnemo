...
function GraphEmptyState({ data }: { data: GraphData }) {
  ...
  return <section className="page graph-state"><div className="graph-state-card graph-state-rich"><span className="graph-state-orbit"><Sparkles size={25} /></span><p className="eyebrow">Memory graph</p><h1>{building ? "Building your first connections" : "Your captures are still independent"}</h1><p>{building ? "Mnemo is embedding your recent clips locally. Related captures will become a map once that work finishes." : "Connections appear when copied items are semantically related or part of the same research thread."}</p><div className="graph-state-details"><span><CircleDot size={15} /> {data.nodes.length} captured memories</span><span><GitBranch size={15} /> {data.unconnectedCount} awaiting a connection</span></div><a className="primary-button" href="/timeline">Browse recent captures</a></div></section>;
}
